...
\textbf{Motivation.} Users frequently issue queries that combine category constraints with specific preparation or exclusion rules. For example, consider the query \textit{``tìm các món gỏi cá không cay''} (find fish salads that are not spicy). Analyzing this query reveals:
\begin{itemize}
    \item \textbf{Class/Method:} \textit{gỏi cá} (fish salad), which implies a combination of a dish type/method (\textit{gỏi}) and an ingredient class (\textit{cá}).
    \item \textbf{Negation:} \textit{không cay} (not spicy).
    \item \textbf{Objective:} Retrieve salad dishes made with fish but strictly excluding spicy ingredients such as chili (\textit{ớt}) or satay (\textit{sa tế}).
\end{itemize}
Dense retrieval cannot resolve these complex requirements without hierarchical expansion, because the query does not lexically match individual descendant ingredient names.

\textbf{Input/Output.} Input is a natural-language query containing one or more class references and optional negation or cooking-method constraints. Output is a ranked list of dishes.
